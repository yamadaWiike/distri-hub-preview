...
function checkMixedVariantsMOQ(
  ...
  const productName =
    matchingItems.length > 0 ? matchingItems[0].name : "Unknown Product";
...
function identifyMixVariantProducts(items: CartItem[]): Map<
  string,
  {
    productId: string;
    province: string;
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-md flex flex-col">
        ...
        {!mixVariantValidation.allValid &&
          mixVariantValidation.results.length > 0 && (
            <div className="mb-4">
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
                <AlertTitle>
                  {lang === "id"
                    ? "MOQ Varian Belum Terpenuhi"
                    : "Mixed Variant MOQ Not Met"}
                </AlertTitle>
                <AlertDescription>
                  <div className="mt-2 space-y-2">
                    {mixVariantValidation.results
                      .filter((r) => !r.isValid)
                      .map((result, idx) => (
                        <div
                          key={`${result.productId}-${result.province}-warning-${idx}`}
                          className="text-sm"
                        >
          ...
          <div className="border-t pt-4">
            ...
            >
              {!mixVariantValidation.allValid
                ? lang === "id"
                  ? "MOQ Varian Belum Terpenuhi"
                  : "Complete Mixed Variant MOQ First"
                : t.checkout}
            </Button>

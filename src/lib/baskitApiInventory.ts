...
export async function getInventory(
  ...
  if (payload.searchKey)
    payload.searchKey.forEach((key) => params.append("searchKey", key));
  if (payload.inventoryId)
    payload.inventoryId.forEach((id) => params.append("inventoryId", id));
  if (payload.unitId)
    payload.unitId.forEach((id) => params.append("unitId", id));
  if (payload.variantId)
    payload.variantId.forEach((id) => params.append("variantId", id));
  if (payload.skus)
    payload.skus.forEach((sku) => params.append("skus", sku));
  if (payload.active !== undefined)
    params.append("active", payload.active.toString());

  const queryString = params.toString();
  const endpoint = `/distributor-hub/inventory-stock${
